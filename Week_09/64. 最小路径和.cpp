...
public:
    int minPathSum(vector<vector<int>>& grid) {
        ...
        unordered_map<string, int> iResult;
...
private:
    ...
    int dfs(int i, int j, int m, int n, vector<vector<int>> &grid, unordered_map<string, int> &iResult) {
        if (i == m || j == n)
            return INT_MAX;
        stringstream ss;
        ss << i << "-" << j;
        if (iResult.find(ss.str()) != iResult.end())
            return iResult[ss.str()];
